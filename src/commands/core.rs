...
fn create_project_overview(roadmap: &crate::model::Roadmap, rask_dir: &Path) -> CommandResult {
    let overview_content = format!(r#"# Project Overview: {}
...
"#, 
        roadmap.title,
        roadmap.tasks.len(),
        roadmap.tasks.iter().filter(|t| t.status == crate::model::TaskStatus::Completed).count(),
        roadmap.tasks.iter().filter(|t| t.status == crate::model::TaskStatus::Pending).count(),
        if roadmap.tasks.is_empty() { 0.0 } else {
            (roadmap.tasks.iter().filter(|t| t.status == crate::model::TaskStatus::Completed).count() as f64 / roadmap.tasks.len() as f64) * 100.0
        },
...
fn create_task_details_file(roadmap: &crate::model::Roadmap, rask_dir: &Path) -> CommandResult {
    let mut details_content = String::from(r#"# Task Details & Metadata
...
"#);
    ...
    for task in &roadmap.tasks {
        ...
        let estimated_hours_str = task.estimated_hours
            .map(|h| h.to_string())
        ...
        details_content.push_str(&format!(r#"## Task #{}: {}
...
---
*Generated by Rask v2.8.2 - AI-Powered Project Management*
"#;
...
fn generate_phase_summary(roadmap: &crate::model::Roadmap) -> String {
    ...
    for (phase, (pending, completed)) in phase_counts {
        summary.push_str(&format!("- **{}**: {} pending, {} completed\n", phase, pending, completed));
    }
...
fn generate_dependencies_summary(roadmap: &crate::model::Roadmap) -> String {
    ...
    if total_deps == 0 {
        ...
        format!("- Total dependency relationships: {}\n- Tasks with dependencies: {}", 
            total_deps,
            roadmap.tasks.iter().filter(|t| !t.dependencies.is_empty()).count()
        )
    }
}
